...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.Log($"dimantion of player is : x {LevelManager.Instance.Player.position.x} , y {LevelManager.Instance.Player.position.y} z {LevelManager.Instance.Player.position.z}");
        Debug.Log($"dimantion of enemy is : x {transform.position.x} , y {transform.position.y} z {transform.position.z}");
       Debug.Log("dis from enemy and hero is : "+ distance);
    ...
    IEnumerator FallAfterDie()
    {
        ...
        {
            LevelManager.Instance.MainCanvas.Find("Vectory").gameObject.SetActive(true);
            Time.timeScale = 0f;
        }
        enemyStatus.Die();
